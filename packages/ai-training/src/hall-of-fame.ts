...
export function admitHallOfFameCandidates(existing: readonly HallOfFameEntry[], candidates: readonly HallOfFameCandidate[]): readonly HallOfFameEntry[] {
  ...
  const admitted = candidates
    .filter((candidate) => candidate.evaluation.invalidCommandRate === 0 && candidate.tickGuardRate === 0 && candidate.evaluation.slotAdjustedScore >= 0 && candidate.evaluation.reliabilityScore >= 800 && !fingerprints.has(candidate.behaviorFingerprint))
